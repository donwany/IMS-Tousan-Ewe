...
class CodecAudioPreprocessor:
    ...
    def __init__(
        ...
        output_sr=16000,
        device="cpu",
        path_to_model="Preprocessing/Codec/encodec_16k_320d.pt",
    ):
        self.device = device
...
if __name__ == "__main__":
    ...
    with torch.inference_mode():
        ...
        ap = CodecAudioPreprocessor(
            input_sr=1, path_to_model="Codec/encodec_16k_320d.pt"
        )
        ...
        print(t1 - t0)
        soundfile.write(
            file=f"../audios/1_reconstructed_in_{t1 - t0}_encodec.wavs",
            data=audio1,
            samplerate=16000,
        )
        soundfile.write(
            file=f"../audios/2_reconstructed_in_{t1 - t0}_encodec.wavs",
            data=audio2,
            samplerate=16000,
        )
        soundfile.write(
            file=f"../audios/3_reconstructed_in_{t1 - t0}_encodec.wavs",
            data=audio3,
            samplerate=16000,
        )
        soundfile.write(
            file=f"../audios/4_reconstructed_in_{t1 - t0}_encodec.wavs",
            data=audio4,
            samplerate=16000,
        )
