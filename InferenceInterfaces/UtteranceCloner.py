...
class UtteranceCloner:
    ...
    def __init__(self, model_id, device, language="eng"):
        ...
        self.aligner_weights = torch.load(
            acoustic_checkpoint_path, map_location=device
        )["asr_model"]
    ...
    def extract_prosody(
        self, transcript, ref_audio_path, lang="eng", on_line_fine_tune=True
    ):
        ...
        with torch.inference_mode():
            speech_timestamps = self.get_speech_timestamps(
                norm_wave, self.silero_model, sampling_rate=16000
            )
        ...
        features = self.ap.audio_to_mel_spec_tensor(
            audio=norm_wave, explicit_sampling_rate=16000
        ).transpose(0, 1)
        ...
        if on_line_fine_tune:
            ...
            for _ in range(steps):
                ...
                loss = self.acoustic_model.ctc_loss(
    ...
    def clone_utterance(
        ...
        """
        self.tts.set_utterance_embedding(
            path_to_reference_audio=path_to_reference_audio_for_voice
        )
        duration, pitch, energy, silence_frames_start, silence_frames_end = (
