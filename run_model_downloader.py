...
def report(block_number, read_size, total_size):
    if block_number % 1000 == 0:
        ...
        print(
            f"{return_to_front}[{'█' * (percent // 2)}{'.' * (50 - (percent // 2))}]",
            end="",
        )
...
def download_models():
    ...
    filename, headers = urllib.request.urlretrieve(
        ...
        filename=os.path.abspath(
            os.path.join(MODELS_DIR, "Embedding", "embedding_function.pt")
        ),
        reporthook=report,
    )
    ...
    filename, headers = urllib.request.urlretrieve(
        ...
        filename=os.path.abspath(
            os.path.join("Preprocessing/Codec", "encodec_16k_320d.pt")
        ),
        reporthook=report,
    )
    ...
    filename, headers = urllib.request.urlretrieve(
        ...
        filename=os.path.abspath(
            os.path.join("Preprocessing/multilinguality", "asp_dict.pkl")
        ),
        reporthook=report,
    )
