...
pipeline_dict = {
    ...
    "finetuning_example_simple_ewe": fine_tuning_example_simple_ewe,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpu_id",
        type=str,
        help="Which GPU(s) to run on. If not specified runs on CPU, but other than for integration tests that doesn't "
             "make much sense.",
        default="cpu",
    )
    ...
    parser.add_argument(
        "--wandb",
        action="store_true",
        help="Whether to use weights and biases to track training runs. Requires you to run wandb login and place "
             "your auth key before.",
        default=False,
    )
    ...
    if args.gpu_id == "cpu":
        ...
        gpu_count = 1  # for technical reasons this is set to one, indicating it's not gpu_count training,
        # even though there is no GPU in this case
    else:
