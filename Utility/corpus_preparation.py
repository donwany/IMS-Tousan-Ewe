...
def prepare_aligner_corpus(
    transcript_dict, corpus_dir, lang, device, phone_input=False, gpu_count=1, rank=0
):
    return CodecAlignerDataset(
        transcript_dict,
        cache_dir=corpus_dir,
        lang=lang,
        loading_processes=10,  # this can be increased for massive clusters, but the overheads that are introduced
        # are kind of not really worth it
        device=device,
...
def prepare_tts_corpus(
    ...
    corpus_dir,
    lang,
    # For small datasets it's best to turn this off and instead inspect the data with the scorer, if there are any
        # issues.
            ...
            if not os.path.exists(os.path.join(aligner_dir, "aligner.pt")):
                ...
                if os.path.exists(os.path.join(MODELS_DIR, "Aligner", "aligner.pt")):
                    train_aligner(
                        ...
                        steps=min(
                            len(aligner_datapoints) // 2, 10000
                        ),  # relatively good finetuning heuristic
                        batch_size=(
                            32
                            if len(aligner_datapoints) > 32
                            else len(aligner_datapoints) // 2
                        ),
                        path_to_checkpoint=os.path.join(
                            MODELS_DIR, "Aligner", "aligner.pt"
                        ),
                        fine_tune=True,
