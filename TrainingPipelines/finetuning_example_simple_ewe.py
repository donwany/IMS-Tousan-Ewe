...
def run(
    ...
    if model_dir is not None:
        ...
        save_dir = os.path.join(
            MODELS_DIR, "ToucanTTS_Finetuning_Ewe"
        )  # RENAME TO SOMETHING MEANINGFUL FOR YOUR DATA
    os.makedirs(save_dir, exist_ok=True)

    train_data = prepare_tts_corpus(
        transcript_dict=build_path_to_transcript_dict_ewe(),
        corpus_dir=os.path.join(PREPROCESSING_DIR, "ewe_integration_test"),
        lang="ewe",
    ...
    train_loop(
        ...
        warmup_steps=500,
        lr=1e-5,  # if you have enough data (over ~1000 datapoints) you can increase this up to 1e-4 and it will
        # still be stable, but learn quicker. DOWNLOAD THESE INITIALIZATION MODELS FROM THE RELEASE PAGE OF THE
        # GITHUB OR RUN THE DOWNLOADER SCRIPT TO GET THEM AUTOMATICALLY
        path_to_checkpoint=(
