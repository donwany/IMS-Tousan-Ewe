...
from Architectures.Vocoder.HiFiGAN_Discriminators import (
...
def run(
    ...
    take_all = (
        False  # use only files with a large enough samplerate or just use all of them
    )
    ...
    selection = file_lists_for_this_run_combined[
        :250000
    ]  # adjust the sample size until it fits into RAM

    fl = list(
        build_path_to_transcript_dict_RAVDESS().keys()
    )  # these two datasets are kind of important to represent some out-of-distribution data for what we expect.
    wav, sr = sf.read(fl[0])
    if sr >= 24000 or take_all:
        selection += fl

    fl = (
        build_file_list_singing_voice_audio_database()
    )  # these two datasets are kind of important to represent some out-of-distribution data for what we expect.
    wav, sr = sf.read(fl[0])
    if sr >= 24000 or take_all:
        selection += fl
