...
class ToucanTTS(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.integrate_language_embedding_into_encoder_out:
            if embedding_integration == "AdaIN":
                self.language_embedding_infusion = AdaIN1d(
                    style_dim=lang_emb_size, num_features=attention_dimension
                )
            elif embedding_integration == "ConditionalLayerNorm":
                self.language_embedding_infusion = ConditionalLayerNorm(
                    speaker_embedding_dim=lang_emb_size, hidden_dim=attention_dimension
                )
            else:
                self.language_embedding_infusion = torch.nn.Linear(
                    attention_dimension + lang_emb_size, attention_dimension
                )

        self.duration_predictor = DurationPredictor(
            idim=attention_dimension,
        ...
        self.decoder = Conformer(
            ...
            use_output_norm=embedding_integration
            not in ["AdaIN", "ConditionalLayerNorm"],
            utt_embed=utt_embed_dim,
            embedding_integration=embedding_integration,
        )
        ...
        self.post_flow = Glow(
            ...
            condition_integration_projection=torch.nn.Conv1d(
                128 + attention_dimension, attention_dimension, 5, padding=2
            ),
        )
    ...
    def forward(
        ...
        """
        outs, glow_loss, predicted_durations, predicted_pitch, predicted_energy = (
            self._forward(
        ...
        # calculate loss
        regression_loss, duration_loss, pitch_loss, energy_loss = self.criterion(
            predicted_features=outs,
    ...
    def _forward(
        ...
        if self.integrate_language_embedding_into_encoder_out:
            ...
            encoded_texts = integrate_with_utt_embed(
                hs=encoded_texts,
                utt_embeddings=lang_embs,
                projection=self.language_embedding_infusion,
        ...
        if is_inference:
            ...
            pitch_predictions = self.pitch_predictor(
                encoded_texts, padding_mask=None, utt_embed=utterance_embedding
            )
            ...
            pitch_predictions = self.pitch_predictor(
                encoded_texts.detach(),
                padding_mask=padding_masks.unsqueeze(-1),
                utt_embed=utterance_embedding,
            )
            energy_predictions = self.energy_predictor(
                encoded_texts,
                padding_mask=padding_masks.unsqueeze(-1),
            ...
            predicted_durations = self.duration_predictor(
                encoded_texts, padding_mask=padding_masks, utt_embed=utterance_embedding
            )
            ...
            enriched_encoded_texts = (
                encoded_texts + embedded_energy_curve + embedded_pitch_curve
            )

            upsampled_enriched_encoded_texts = self.length_regulator(
                enriched_encoded_texts, gold_durations
            )
        ...
        if is_inference:
            if run_glow:
                refined_codec_frames = self.post_flow(
                    tgt_mels=gold_speech,
                    infer=is_inference,
                    mel_out=preliminary_spectrogram,
    ...
    def inference(
        ...
        outs, duration_predictions, pitch_predictions, energy_predictions = (
            self._forward(
                text_pseudobatched,
                ilens,
