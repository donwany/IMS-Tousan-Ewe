...
class ToucanTTS(torch.nn.Module):
    ...
    def _forward(
        ...
        predicted_durations = (
            self.duration_predictor.inference(
                encoded_texts, padding_mask=None, utt_embed=utterance_embedding
            )
        ...
        pitch_predictions = make_near_zero_to_zero(
            pitch_predictions.squeeze(0)
        ).unsqueeze(0)
        energy_predictions = make_near_zero_to_zero(
            energy_predictions.squeeze(0)
        ).unsqueeze(0)
