...
from Architectures.ToucanTTS.LanguageEmbeddingSpaceStructureLoss import (
...
def train_loop(
    ...
    optimizer = torch.optim.Adam(
        [p for name, p in model.named_parameters() if "post_flow" not in name], lr=lr
    )
    ...
    flow_scheduler = WarmupScheduler(
        flow_optimizer, peak_lr=lr, warmup_steps=(warmup_steps // 4), max_steps=steps
    )
    ...
    for step_counter in tqdm(range(steps_run_previously, steps)):
        ...
        if use_less_loss:
            ...
            language_ids = random.sample(valid_language_ids, batch_size)
            language_embeddings_random = model.encoder.language_embedding(
                torch.LongTensor(language_ids).to(device)
            )
            less_value = less_loss(
                lang_ids.cpu().squeeze().tolist() + language_ids,
                torch.cat(
                    [language_embeddings_seen, language_embeddings_random], dim=0
                ),
            )
        ...
        if (
            glow_loss is not None
        ):  # even if run_glow is true, this can still happen if the log prob cannot be calculated.
            ...
            if glow_loss < 0.0:
                ...
                glow_losses_total.append(
                    0.1
                )  # just to avoid super large numbers during plotting that mess up the scaling
        else:
        ...
        torch.nn.utils.clip_grad_norm_(
            [p for name, p in model.named_parameters() if "post_flow" not in name],
            1.0,
        ...
        if glow_loss is not None:
            torch.nn.utils.clip_grad_norm_(
                model.post_flow.parameters(), 1.0, error_if_nonfinite=False
            )
            flow_optimizer.step()
            flow_scheduler.step()
                    ...
                    wandb.log(
                        {
                            ...
                            "glow_loss": round(
                                sum(glow_losses_total) / len(glow_losses_total), 5
                            ),
                            "duration_loss": round(
                            ...
                            "embedding_structure_loss": (
                                0.0
                                if len(less_losses_total) == 0
                                else round(
                                    sum(less_losses_total) / len(less_losses_total), 5
                                )
                            ),
