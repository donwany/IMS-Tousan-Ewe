...
from Architectures.ToucanTTS.StochasticToucanTTS.StochasticToucanTTSLoss import (
    StochasticToucanTTSLoss,
)
from Architectures.ToucanTTS.StochasticToucanTTS.StochasticVariancePredictor import (
    StochasticVariancePredictor,
)
...
class StochasticToucanTTS(torch.nn.Module):
    ...
    def __init__(
        ...
        self.duration_flow = StochasticVariancePredictor(
        ...
        self.pitch_flow = StochasticVariancePredictor(
            in_channels=attention_dimension,
            kernel_size=5,
            p_dropout=0.5,
            n_flows=6,
        ...
        self.energy_flow = StochasticVariancePredictor(
            in_channels=attention_dimension,
            kernel_size=3,
            p_dropout=0.5,
            n_flows=3,
        ...
        self.post_flow = Glow(
            ...
            condition_integration_projection=torch.nn.Conv1d(
                output_spectrogram_channels + attention_dimension,
                attention_dimension,
                5,
                padding=2,
            ),
        ...
        if lang_embs is not None:
            torch.nn.init.normal_(
                self.encoder.language_embedding.weight,
                mean=0,
                std=attention_dimension**-0.5,
            )
    ...
    def forward(
        ...
        """
        before_outs, after_outs, duration_loss, pitch_loss, energy_loss, glow_loss = (
            self._forward(
    ...
    def _forward(
        ...
        if is_inference:
            variance_mask = torch.ones(
                size=[text_tensors.size(1)], device=text_tensors.device
            )
            ...
            upsampled_enriched_encoded_texts = self.length_regulator(
                encoded_texts, predicted_durations
            )
            ...
            scaled_pitch_targets[idx] = torch.exp(
                gold_pitch[idx]
            )  # we scale up, so that the log in the flow can handle the value ranges better.
            pitch_flow_loss = torch.sum(
                self.pitch_flow(
                    encoded_texts.transpose(1, 2).detach(),
                    pitch_mask,
                    w=scaled_pitch_targets.transpose(1, 2),
                    g=utterance_embedding.unsqueeze(-1),
                    reverse=False,
                )
            )
            pitch_flow_loss = torch.sum(
                pitch_flow_loss / torch.sum(pitch_mask)
            )  # weighted masking
            embedded_pitch_curve = self.pitch_embed(
            ...
            scaled_energy_targets[idx] = torch.exp(
                gold_energy[idx]
            )  # we scale up, so that the log in the flow can handle the value ranges better.
            energy_flow_loss = torch.sum(
                self.energy_flow(
                    encoded_texts.transpose(1, 2).detach(),
                    energy_mask,
                    w=scaled_energy_targets.transpose(1, 2),
                    g=utterance_embedding.unsqueeze(-1),
                    reverse=False,
                )
            )
            energy_flow_loss = torch.sum(
                energy_flow_loss / torch.sum(energy_mask)
            )  # weighted masking
            embedded_energy_curve = self.energy_embed(
            ...
            duration_flow_loss = torch.sum(
                self.duration_flow(
                    encoded_texts.transpose(1, 2).detach(),
                    duration_mask,
                    w=duration_targets.transpose(1, 2),
                    g=utterance_embedding.unsqueeze(-1),
                    reverse=False,
                )
            )
            duration_flow_loss = torch.sum(
                duration_flow_loss / torch.sum(duration_mask)
            )  # weighted masking
        ...
        decoder_masks = (
            make_non_pad_mask(speech_lengths, device=speech_lengths.device).unsqueeze(
                -2
            )
            if speech_lengths is not None and not is_inference
            else None
        )
        ...
        decoded_spectrogram = self.feat_out(decoded_speech).view(
            decoded_speech.size(0), -1, self.output_spectrogram_channels
        )
        ...
        if run_glow:
            if is_inference:
                refined_spectrogram = self.post_flow(
                    tgt_mels=None,
                    infer=is_inference,
...
if __name__ == "__main__":
    ...
    dummy_speech_batch = torch.randn(
        [3, 30, 80]
    ...
    dummy_pitch = torch.Tensor(
        [[[1.0], [0.0], [0.0]], [[1.1], [1.2], [0.8]], [[1.1], [1.2], [0.8]]]
    )
    dummy_energy = torch.Tensor(
    ...
    dummy_pitch = torch.Tensor([[[1.0], [0.0], [0.0]], [[1.1], [1.2], [0.8]]])
    dummy_energy = torch.Tensor([[[1.0], [1.3], [0.0]], [[1.1], [1.4], [0.8]]])
    ...
    dummy_text_batch = torch.randint(
        low=0, high=2, size=[12, 62]
    ).float()  # [Sequence Length, Features per Phone]
