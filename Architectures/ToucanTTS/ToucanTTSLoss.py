...
class ToucanTTSLoss(torch.nn.Module):
    ...
    def forward(
        ...
        out_masks = (
            make_non_pad_mask(features_lengths).unsqueeze(-1).to(gold_features.device)
        )
        ...
        pitch_loss = (
            pitch_loss.mul(variance_weights).masked_select(variance_masks).sum()
        )
        energy_loss = (
            energy_loss.mul(variance_weights).masked_select(variance_masks).sum()
        )
