...
class CodecAlignerDataset(Dataset):
    ...
    def __init__(
        ...
        cache = torch.load(
            os.path.join(self.cache_dir, "aligner_train_cache.pt"), map_location="cpu"
        )
    ...
    def _build_dataset_cache(
        ...
        for key_split in key_splits:
            process_list.append(
                Process(
                    target=self._cache_builder_process,
                    args=(
                        key_split,
                        lang,
                        min_len_in_seconds,
        ...
        text_tensors = [
            torch.ShortTensor(x[0]) for x in self.result_pool
        ]  # turn everything back to tensors (had to turn it to np arrays to avoid multiprocessing issues)
        ...
        torch.save(
            (self.datapoints, None, self.speaker_embeddings, filepaths),
    ...
    def _cache_builder_process(
        ...
        for path in tqdm(path_list):
            ...
            if not (min_len <= dur_in_seconds <= max_len):
                if verbose:
                    print(
                        f"Excluding {path} because of its duration of {round(dur_in_seconds, 2)} seconds."
                    )
                continue
            ...
            with torch.no_grad():
                speech_timestamps = get_speech_timestamps(
                    norm_wave, silero_model, sampling_rate=16000
                )
            try:
                result = norm_wave[
            ...
            try:
                try:
                    cached_text = (
                        tf.string_to_tensor(
                            transcript, handle_missing=False, input_phonemes=phone_input
                        )
                        .squeeze(0)
                        .cpu()
                        .numpy()
                    ...
                    cached_text = (
                        tf.string_to_tensor(
                            transcript, handle_missing=True, input_phonemes=phone_input
                        )
                        .squeeze(0)
                        .cpu()
                        .numpy()
            ...
            process_internal_dataset_chunk.append(
                [cached_text, cached_speech, result.cpu().detach().numpy(), path]
            )
    ...
    def __getitem__(self, index):
        ...
        return tokens, token_len, codes, None, self.speaker_embeddings[index]
