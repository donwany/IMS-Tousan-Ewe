...
class Reconstructor(torch.nn.Module):
    ...
    def __init__(
        self, n_features=128, num_symbols=145, speaker_embedding_dim=192, lstm_dim=256
    ):
        super().__init__()
        self.in_proj = torch.nn.Linear(num_symbols + speaker_embedding_dim, lstm_dim)
        self.rnn1 = torch.nn.LSTM(
            lstm_dim, lstm_dim, batch_first=True, bidirectional=True
        )
    ...
    def forward(self, x, lens, ys):
        ...
        l1_loss = (
            self.l1_criterion(x, ys).mul(out_weights).masked_select(out_masks).sum()
        )
        l2_loss = (
            self.l2_criterion(x, ys).mul(out_weights).masked_select(out_masks).sum()
        )
