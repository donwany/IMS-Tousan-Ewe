...
class WassersteinGanQuadraticCost:
    ...
    def __init__(
        ...
        self.c, self.A, self.pStart = self._prepare_linear_programming_solver_(
            self.batch_size
        )

    def _build_lr_scheduler_(self, optimizer, milestones, lr_anneal, last_epoch=-1):
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, milestones, gamma=lr_anneal, last_epoch=-1
        )
        return scheduler
    ...
    def _prepare_linear_programming_solver_(self, batch_size):
        ...
        for i in range(1, batch_size):
            Ai = spmatrix(
                1.0, range(batch_size), [i] * batch_size, (batch_size, batch_size)
            )
            A = sparse([A, Ai])

        D = spmatrix(
            -1.0, range(batch_size), range(batch_size), (batch_size, batch_size)
        )
        DM = D
        for i in range(1, batch_size):
            DM = sparse([DM, D])
    ...
    def _optimal_transport_regularization_(self, output_fake, fake, real_fake_diff):
        ...
        RegLoss = (
            ...
            * (
                (
                    gradients.view(n, -1).norm(dim=1) / (2 * self.Kr)
                    - self.Kr / 2 * real_fake_diff.view(n, -1).norm(dim=1)
                ).pow(2)
            ).mean()
        )
    ...
    def _critic_deep_regression_(self, images, opt_iterations=1):
        ...
        for i in range(opt_iterations):
            ...
            L2LossD_real = self.criterion(
                output_R_mean[0], target[: self.batch_size].mean()
            )
            L2LossD_fake = self.criterion(output_fake, target[self.batch_size :])
            L2LossD = 0.5 * L2LossD_real + 0.5 * L2LossD_fake

            reg_loss_D = self._optimal_transport_regularization_(
                output_fake, generated_data, real_fake_diff
            )
    ...
    def sample_generator(self, num_samples, nograd=False, return_intermediate=False):
        self.G.eval()
        if isinstance(self.G, torch.nn.parallel.DataParallel):
            latent_samples = self.G.module.sample_latent(
                num_samples, self.G.module.z_dim
            )
        else:
        ...
        if nograd:
            with torch.no_grad():
                generated_data = self.G(
                    latent_samples, return_intermediate=return_intermediate
                )
        else:
