...
class HiFiGANScaleDiscriminator(torch.nn.Module):
    ...
    def __init__(
        ...
        self.post_conv = SANConv1d(
            out_chs, out_channels, kernel_sizes[3], padding=(kernel_sizes[3] - 1) // 2
        )
...
class AvocodoHiFiGANJointDiscriminator(torch.nn.Module):
    ...
    def forward(
        ...
        mcmbd_outs, mcmbd_feats = self.mcmbd(
            wave_final=wave,
        ...
        return (
            msd_outs + mpd_outs + mcmbd_outs + msbd_outs,
            msd_feats + mpd_feats + mcmbd_feats + msbd_feats,
        )
